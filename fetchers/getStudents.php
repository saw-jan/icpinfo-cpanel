...
function getStudent(){
  global $con;
  global $num;
  $query = "SELECT * FROM icp_students";
  ...
  if($row_num>0){
    while($row = mysqli_fetch_assoc($run)){
      ...
      <td style='font-size:12px;padding:8px;border-right:1px solid #444; border-top:1px solid #444;'>$gender</td>
      <td style='font-size:12px;padding:8px;border-right:1px solid #444; border-top:1px solid #444;'>$faculty</td>
      <td style='font-size:12px;padding:8px;border-right:1px solid #444; border-top:1px solid #444;'>
      <a href='../fetchers/delStudent.php?id=".$id."'><img style='width:20px;' src='../assets/delete.png'/></a></td>
      </tr>";
    }
    $num = $row_num;
  }
